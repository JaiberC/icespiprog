...
///////////////////////////librerías adicionales usadas en iceprog /////////////////////////////////
#include <stdbool.h>
#include <string.h>
#include <errno.h>
///////////////////////////librerías adicionales usadas en iceprog /////////////////////////////////
...
void blockErase(int fd, int addr){
	int ret;
	uint8_t tx[] = {BLOCK_ERASE_64K, (uint8_t)(addr >> 16), (uint8_t)(addr >> 8), (uint8_t)addr};
	uint8_t rx[ARRAY_SIZE(tx)] = {0, };
	struct spi_ioc_transfer tr = {
		.tx_buf = (unsigned long)tx,
		.rx_buf = (unsigned long)rx,
...
int main(int argc, char *argv[])
{
	...
	blockErase(fd,0x00);//block erase 64kb	
	sleep(5);
	writeEnable(fd);//write enable
	blockErase(fd,0x010000);//block erase 64kb	
	sleep(5);
	//writeEnable(fd);
	//WriteByte(fd);
